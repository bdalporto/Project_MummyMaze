...
handleEvent g (AppEvent Counter)                    = continue $ moveMummies g
...
moveMummies g = g {_mummies = (map (moveMummy ec g) mummies)}
...
moveExplorer d g = g {_explorer = new_coords, _mummies = setMummyCounters g }
    where 
...
drawUI g = (drawCharacters g) ++ [(drawGrid g)]
